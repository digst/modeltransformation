...
<h1>Concept List for: <xsl:call-template name="modellabel"/>
</h1>
<!--
...
<hr/>
	...
	<!-- Show model metadata-->
			...
			<div class="table">
				...
				<dt>Namespace prefix:</dt><dd><xsl:value-of select="vann:preferredNamespacePrefix"/></dd>
				</xsl:if>
				<dt>Model name:</dt>
				<dd>	
				<xsl:call-template name="modellabel"> </xsl:call-template> 
				</dd>			 
				<dt>Model publisher:</dt><dd><xsl:for-each select="dct:publisher"> <xsl:value-of select="."/></xsl:for-each></dd> 
				<dt>Responsible entity:</dt><dd><xsl:for-each select="frbr:responsibleEntity"> <xsl:value-of select="."/></xsl:for-each></dd> 	
				<dt>Version number:</dt><dd><xsl:value-of select="owl:versionInfo"/></dd>
				<dt>Modified:</dt><dd><xsl:value-of select="dct:modified"/></dd>
				<dt>Model status:</dt><dd><xsl:value-of select="adms:status"/> &#160;<xsl:value-of select="adms:status/@rdf:resource"/></dd>
			<xsl:if test="voag:hasApprovalStatus"><dt>Approval status:</dt><dd><xsl:value-of select="voag:hasApprovalStatus"/>&#160;<xsl:value-of select="voag:hasApprovalStatus/@rdf:resource"/></dd></xsl:if>
			<xsl:for-each select="dct:theme"><dt>Theme:</dt><dd><xsl:value-of select="."/></dd> </xsl:for-each>	 
			<xsl:if test="cv:hasLegalResource"><dt>Legal source:</dt><dd><xsl:for-each select="cv:hasLegalResource"> <xsl:value-of select="."/>; </xsl:for-each></dd></xsl:if>
			<xsl:if test="dct:source"><dt>Source:</dt><dd><xsl:for-each select="dct:source"> <xsl:value-of select="."/>; </xsl:for-each></dd></xsl:if>	
			<xsl:if test="dct:language"><dt>Language:</dt><dd><xsl:for-each select="dct:language"> <xsl:value-of select="."/>; </xsl:for-each></dd></xsl:if>			
				</dl>
			</div>
...
<!--Show Concepts  -->	
		<xsl:if test="rdfs:Class|owl:Class|rdf:Property|owl:AnnotationProperty|owl:ObjectProperty|owl:DatatypeProperty|rdf:Description|skos:Concept">
...
<xsl:template name="listheader">
		<tr><th>(Preferred) term (en)</th><th>Accepted term (en)</th><th>Deprecated term (en)</th><th>Definition (en) </th><th>Example (en)</th><th>Comment (en)</th><th>Legal source</th><th>Source</th><th>Belongs to model theme</th></tr>
</xsl:template>
...
<xsl:template match="skos:prefLabel">				
		   <xsl:choose>
			 <xsl:when test="@xml:lang!=''">							 							
				<xsl:if test="@xml:lang='en'"><xsl:value-of select="."/></xsl:if>	
					   <xsl:choose>
						 <xsl:when test="../rdfs:label[@xml:lang!='']">
						 <xsl:if test="../rdfs:label[@xml:lang='en']"><xsl:value-of select="../rdfs:label[@xml:lang='en']"/></xsl:if>	</xsl:when>						 
						 <xsl:otherwise>	
						 <xsl:if test="../rdfs:label"><xsl:value-of select="../rdfs:label"/><xsl:value-of select="../rdfs:label[@xml:lang='en']"/></xsl:if>								
						</xsl:otherwise>			 
					   </xsl:choose>
			 </xsl:when>						   
			 <xsl:otherwise>	
